        ...
        if (resJson.values) {
          ...
          resJson.values.forEach(item => {
            if (!arr.some(element => element.name === item[3])) {
              const count = resJson.values.filter(
                element => element[3] === item[3],
              ).length;
              arr.push({name: item[3], total: count});
            }
          });
          this.setState({
            tableData: resJson.values,
            errMessage: '',
            refresh: false,
            teamArr: arr,
          });
      ...
      <SafeAreaView style={{flex: 1}}>
          ...
          <View
            ...
            <View style={styles.inputWrapper}>
              ...
              <TouchableOpacity
                style={styles.icon}
                onPress={() => {
                  search !== '' ? this.onSearch('') : this.focus();
                }}>
                <Icon
        ...
        <View
          ...
          <TouchableOpacity
            ...
            style={[
              styles.tab,
              !userListTab ? {borderBottomColor: colors.burntOrange} : {},
            ]}>
            <Text style={!userListTab ? styles.activeText : {}}>
              Department ({teamArr.length})
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() =>
              !userListTab
                ? this.setState(
                    {userListTab: true, search: '', searchResult: []},
                    () => this.getUserTeam('All'),
                  )
                : null
            }
            style={[
