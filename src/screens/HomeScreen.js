...
import { SheetID, SheetName, } from '../utils'
...
class HomeScreen extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps && nextProps.dataReloadNoti && nextProps.dataReloadNoti.isReload) {
      if (nextProps.dataReloadNoti && nextProps.dataReloadNoti.item) {
        if (nextProps.dataReloadNoti.item.isRead) {
          this.setState({
            countNoti: this.state.countNoti - 1
          }, () => {
  ...
  getData = () => {
    console.log('getData')
    const url = 'https://sheets.googleapis.com/v4/spreadsheets/' + SheetID + '/values/' + SheetName + '!A9:M/?key=AIzaSyDMzmnG1bYziDhPlTZCUJZWcV0mSzpwX-c';
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        {/* <wwww backgroundColor={colors.blue} barStyle="light-content" /> */}
        ...
        <MainHeaderComponent countNoti={this.state.countNoti} onPressRightButton={this.onPressRightButton} onPressLeftButton={this.toggleDrawerMenu} />
...
const mapStateToProps = (state) => {
  const { isAuthenticated, user, dataReloadNoti } = state;
  return {
    isAuthenticated, user, dataReloadNoti
  };
};
